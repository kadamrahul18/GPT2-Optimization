...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        if p.dtype == tl.float16:
            tl.static_print("Data type of p: float16")
        elif p.dtype == tl.float32:
            tl.static_print("Data type of p: float32")
        # Add more conditions for other types if needed

        if v.dtype == tl.float16:
            tl.static_print("Data type of v: float16")
        elif v.dtype == tl.float32:
            tl.static_print("Data type of v: float32")

        if acc.dtype == tl.float16:
            tl.static_print("Data type of acc: float16")
        elif acc.dtype == tl.float32:
            tl.static_print("Data type of acc: float32")
